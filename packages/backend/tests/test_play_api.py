...
Exercises the play API end-to-end through the pytest ``client`` fixture (a
:class:`fastapi.testclient.TestClient` over an isolated, rolled-back Postgres
transaction) and — the critical guarantee — asserts that *no* ``/v1/play``
response ever leaks an engine internal (``theta``, ``mastered_count``, ``total``,
the per-trial score ``s``, or predicted success ``E``).
...
``MATH_PRACTICE_TEST_DATABASE_URL``):
    ...
    uv run --package math-practice-backend --extra dev --extra postgres \
        python -m pytest packages/backend/tests/test_play_api.py -q
"""
...
def test_play_api_flow_and_no_leak(client: TestClient) -> None:
